...
export const installEdgeBackGesture = (container: HTMLElement, canStart: () => boolean, handler: EdgeGestureHandler): (() => void) => {
  ...
  container.addEventListener('pointerdown',down,{passive:true,capture:true});container.addEventListener('pointermove',move,{passive:false,capture:true});container.addEventListener('pointerup',up,{passive:false,capture:true});container.addEventListener('pointercancel',cancelPointer,{passive:true,capture:true});window.addEventListener('blur',blur);
  const cleanup=()=>{container.removeEventListener('pointerdown',down,true);container.removeEventListener('pointermove',move,true);container.removeEventListener('pointerup',up,true);container.removeEventListener('pointercancel',cancelPointer,true);window.removeEventListener('blur',blur);if(state.tracking)cancel();if(installed.get(container)===cleanup)installed.delete(container);};installed.set(container,cleanup);return cleanup;
};
