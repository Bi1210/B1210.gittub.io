...
const EchoesApp = lazyApp(() => import('../apps/EchoesApp'));
...
const APP_PRELOAD_ORDER: PreloadableLazy[] = [
  ...
  StudyApp, GameApp, EchoesApp, NovelApp, BankApp, WorldbookApp, MemoryPalaceApp, HandbookApp,
...
const APP_BY_ID: Partial<Record<AppID, PreloadableLazy>> = {
  ...
  [AppID.FAQ]: FAQApp, [AppID.Game]: GameApp, [AppID.Echoes]: EchoesApp, [AppID.Worldbook]: WorldbookApp,
...
const PhoneShell: React.FC = () => {
  ...
  // Interactive back uses the same navigation protocol as the native back button.
  // If an App has an internal page/modal handler, it gets first refusal; otherwise
  // OSContext falls back to closing the current App. This keeps one return path.
      ...
      edgeSwipeTimerRef.current = window.setTimeout(() => {
        ...
        if (commit) {
          try {
            ...
            // Navigation must never turn a gesture callback into an App crash.
            console.error('[EdgeBack] closeApp failed', error);
          }
        }
        reset();
      }, 220);
  ...
  const renderApp = (appId: AppID = activeApp) => {
    switch (appId) {
      ...
      case AppID.Echoes: return <EchoesApp />; 
